...
function pageExams() {
  ...
  return `
    ...
    ${next ? examHero(next) : ''}
    ${upcoming.filter(e => e !== next).length ? `<div class="sg-section-label">Also coming up</div><div class="grid grid-3 mb-16 exam-grid">${upcoming.filter(e => e !== next).map(examCard).join('')}</div>` : ''}
    ...
    ${past.length ? `<details class="todo-done mt-16"><summary><span>Past exams</span><span class="assign-count">${past.length}</span></summary><div class="card assign-list">${past.map(e => {
